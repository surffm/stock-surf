...
import { Sparkles, TrendingUp, Plus, X, Info } from 'lucide-react';
...
const WaveStockSurfer = () => {
  ...
  const characters = useMemo(() => [
    { id: 'surfer', name: 'Wave Warrior', emoji: '🏄‍♂️', unlocked: true, color: '#FF6B35' },
    { id: 'ninja', name: 'Surf Ninja', emoji: '🥷', unlocked: true, color: '#4ECDC4' },
    { id: 'dolphin', name: 'Wave Dolphin', emoji: '🐬', unlocked: false, unlock: 'Reach 5 streak', color: '#FFE66D' },
    { id: 'cat', name: 'Surf Cat', emoji: '🐱', unlocked: false, unlock: 'Score 1000+', color: '#95E1D3' },
    { id: 'lightning', name: 'Crest Legend', emoji: '⚡', unlocked: false, unlock: 'Reach 10 streak', color: '#F38181' },
    { id: 'star', name: 'Beach Boss', emoji: '🌟', unlocked: false, unlock: 'Reach 15 streak', color: '#AA96DA' }
  ], []);
  ...
  const [unlockedChars, setUnlockedChars] = useState(['surfer', 'ninja']);
  const [powerUpCount, setPowerUpCount] = useState(0);

  const FINNHUB_API = 'd49emh9r01qshn3lui9gd49emh9r01qshn3luia0';
  ...
  const fetchHistoricalData = useCallback(async (symbol) => {
    try {
      ...
      const response = await fetch(
        `https://finnhub.io/api/v1/stock/candle?symbol=${symbol}&resolution=D&from=${from}&to=${to}&token=${FINNHUB_API}`
      );
  ...
  const initialStocks = useMemo(() => [
    { symbol: 'AAPL', color: '#60A5FA', selectedChar: 'surfer', chartData: null },
    { symbol: 'GOOGL', color: '#EC4899', selectedChar: 'ninja', chartData: null }
  ], []);
  
  const [stocks, setStocks] = useState(initialStocks);
  const [selectedStock, setSelectedStock] = useState('AAPL');
  const [selectedChars, setSelectedChars] = useState({ AAPL: 'surfer', GOOGL: 'ninja' });
  ...
  const fetchStockPrices = useCallback(async () => {
    ...
    for (const stock of stocks) {
      try {
        const response = await fetch(
          `https://finnhub.io/api/v1/quote?symbol=${stock.symbol}&token=${FINNHUB_API}`
        );
        
        if (response.ok) {
          const data = await response.json();
          if (data.c && data.c > 0) {
            newPrices[stock.symbol] = data.c;
  ...
  useEffect(() => {
    const initializeCharts = async () => {
      for (const stock of stocks) {
        ...
        seriesRef.current[stock.symbol] = candlestickSeries;
  ...
  useEffect(() => {
    ...
    if (streak >= 5 && !newUnlocked.includes('dolphin')) newUnlocked.push('dolphin');
    if (score >= 1000 && !newUnlocked.includes('cat')) newUnlocked.push('cat');
    if (streak >= 10 && !newUnlocked.includes('lightning')) newUnlocked.push('lightning');
    if (streak >= 15 && !newUnlocked.includes('star')) newUnlocked.push('star');
    setUnlockedChars(newUnlocked);
  }, [streak, score, unlockedChars]);
  ...
  const handleAddStock = useCallback(async (symbol, color) => {
    ...
    setSelectedChars(prev => ({ ...prev, [symbol.toUpperCase()]: 'surfer' }));
    setSurferPositions(prev => ({ 
      ...prev, 
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-6">
          ...
          <p className="text-blue-200 text-lg">
            {isMobile ? 'Touch to surf!' : 'Use arrow keys & SPACE to surf real candlestick charts!'}
          </p>
        </div>
        ...
        {stocks.length === 0 ? (
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-12 border-2 border-white/20 text-center mb-6">
            ...
            <p className="text-blue-200 mb-6">Add stocks to see real candlestick charts</p>
            <button
              onClick={() => {
                setShowMenu(true);
                    ...
                    {surferPos && (
                      <div
                        ...
                        style={{
                          ...
                          top: `${surferPos.y * 100}%`,
                          transform: `translate(-50%, -50%) ${surferPos.jumping ? 'translateY(-30px)' : ''} ${surferPos.spinning ? 'rotate(360deg)' : ''}`,
                          fontSize: '32px',
              ...
              <div className="p-6 max-h-[70vh] overflow-y-auto">
                {activeMenuTab === 'add' && (
                  <div>
                    ...
                    <p className="text-blue-200 mb-4">Enter any stock symbol to add real candlestick chart data</p>
                ...
                {activeMenuTab === 'trending' && (
                  <div>
                    ...
                    <p className="text-blue-200 mb-4">Click to add popular stocks with real candlestick charts</p>
        ...
        <div className="text-center text-blue-200 text-sm">
          <div>💡 Surf real stock candlestick charts powered by TradingView & Finnhub</div>
